...
class TestFibonacciCalculatorBasic:
    ...
    def test_calculate_by_index_twenty(self):
    ...
    def test_calculate_by_index_sequence(self):
        ...
        for i, expected_val in enumerate(expected):
            ...
            assert result == expected_val, f"F({i}) should be {expected_val}, got {result}"
...
class TestFibonacciCalculatorEdgeCases:
    ...
    def test_calculate_by_index_negative_large_raises_error(self):
    ...
    def test_calculate_by_index_very_large_value(self):
        """Test calculation for very large index (F(1000))."""
        calc = FibonacciCalculator()
        result = calc.calculate_by_index(1000)
        # F(1000) should be a very large number
        assert result > 0
        assert isinstance(result, int)
        # Verify it's correct by checking known value
        # F(100) = 354224848179261915075
        if hasattr(calc, 'calculate_by_index'):
            f100 = calc.calculate_by_index(100)
            assert f100 == 354224848179261915075
    
    def test_calculate_by_index_handles_large_numbers(self):
        """Test that calculator handles very large Fibonacci numbers."""
        calc = FibonacciCalculator()
        # F(200) should be computable
        result = calc.calculate_by_index(200)
        assert result > 0
        assert isinstance(result, int)
        # Verify it's a valid large integer
        assert result.bit_length() > 100
    
    def test_calculate_by_index_non_integer_raises_error(self):
        """Test that non-integer input raises appropriate error."""
        calc = FibonacciCalculator()
        # Python's type system will catch this at runtime if type hints are enforced
        # But we should test behavior
        with pytest.raises((TypeError, AttributeError)):
            calc.calculate_by_index(10.5)  # type: ignore
    
    def test_calculate_by_index_zero_is_valid(self):
        """Test that zero is a valid input (edge case)."""
        calc = FibonacciCalculator()
        result = calc.calculate_by_index(0)
        assert result == 0
    
    def test_calculate_by_index_one_is_valid(self):
...
class TestFibonacciCalculatorByDigits:
    ...
    def test_calculate_by_digits_one_digit(self):
        ...
        # F(0)=0 has 1 digit, so it's the first with at least 1 digit
        assert result == 0
    ...
    def test_calculate_by_digits_two_digits(self):
        ...
        # First Fibonacci with 2 digits is F(7)=13
        assert result == 13
    ...
    def test_calculate_by_digits_three_digits(self):
        ...
        # First Fibonacci with 3 digits is F(12)=144
        assert result == 144
    ...
    def test_calculate_by_digits_four_digits(self):
        ...
        # First Fibonacci with 4 digits is F(17)=1597
        assert result == 1597
